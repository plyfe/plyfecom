...
var renderRightRailContent2 = function(){
  var url = window.location.href;
  if (url.indexOf("blogs/economist-explains") > -1 == false){
    var html = [
      '<div style="display:none;" id="block-ec_blogs-ec_blogs_block_recent" class="block block-ec_blogs mv-block2">',
        '<div class="content clearfix">',
          '<div class="title mv-sponsor-title">',
            '<h6>Sponsor Content</h6>',
          '</div>',
          '<dl>',
            '<div id="mediavoice-placeholder3" style="display:none"></div>',
            '<div id="mediavoice-placeholder4" style="display:none"></div>',
          '</dl>',
        '</div>',
      '</div>',
      ].join('');
    $(html).insertAfter($('#product-events .product-section:last-child'));
  }
}();
...
try {
  ...
  var compiledTemplate0 = '';
  var theLabel1 = "Right Rail- Products & events - 1 ";
  	theLabel1 = theLabel1.concat(prop4, " - ", channel);
  var theLabel2 = "Right Rail- Products & events - 2 ";
  	theLabel2 = theLabel2.concat(prop4, " - ", channel);
...
window.NATIVEADS.onReady = function(ads) {
  ...
  ads.setPropertyID("NA-ECON-11235955");
  ads.setSecondaryPageURL("/native-ad");
  ads.enableMOAT(true, true);
	
  ads.insertPreview({
    label: theLabel1,
    unit: {"server":"dfp","id":"/5605/NativeAds","size":"2x2","targets":{"pos":"productevents","prop41":prop41,"prop42":prop42,"order":"1"}},
    location: "#mediavoice-placeholder3",
    infoText: "",
    infoButtonText: "",
    template: compiledTemplate0,
    onRender: function($element) { },
    onFill: function(data) { $( ".mv-block2").css("display", "");},
  ...
  ads.insertPreview({
    label: theLabel2,
    unit: {"server":"dfp","id":"/5605/NativeAds","size":"2x2","targets":{"pos":"productevents","prop41":prop41,"prop42":prop42,"order":"2"}},
    location: "#mediavoice-placeholder4",
    infoText: "",
    infoButtonText: "",
    template: compiledTemplate0,
    onRender: function($element) { },
  ...
  ads.configureSecondaryPage({
    binding: {
      sponsor: {
        link: "#sponsor-link",
        logo: "#sponsor-logo",
        name: "#column-content .typog-content-header.main-content-header .fly-title"
      },
      title: "#column-content .typog-content-header.main-content-header .headline",
      summary: "#summary",
      content: "#column-content .main-content .content-full",
      author: "",
      pubDate: "",
      image: {
        href: "#column-content .main-content .content-image-full",
        caption: "#media-caption",
        credits: "#media-credits"
      }
    },
    onFill: function(data) {
    if (window.location.href.indexOf("native-ad") > -1){
      var convertedDate = new Date(data.pubDate);
      var wkdy = convertedDate.getDay();
      var d = convertedDate.getDate();
      var m = convertedDate.getMonth();
      var y = convertedDate.getFullYear();
      var h = convertedDate.getHours();
      var mins = convertedDate.getMinutes();
      var dateSuffix = "";
      ...
      var weekday = weekdays[wkdy];
      var formattedDate = month + " " + d + dateSuffix + " " + y + ", " + h + ":" + mins;
      $("#column-content .floatleft.light-grey .date-created").html(formattedDate);
      if (data.author.name !== ""){
        $("#column-content .floatleft.light-grey .location").prepend("by " + data.author.name);
      }
    }},
    onRender: function() {
      if (window.location.href.indexOf("native-ad") > -1){
      	$("#column-content .typog-content-header.main-content-header .fly-title").prepend("Sponsored by ");
      	ads.injectCSS(".main-content .content-image-full img { width: 595px; height: auto; padding-bottom: 10px; } .main-content .content-image-full { margin: 0 15px 0 0; float: left; }", "head");
       }
    },
    onError: function(error) { },
    track: function() { }
...
<div class="views-row views-row-1 views-row-odd views-row-first omniture-tagged omniture-tagged-6 mv-ad">
  <div class="latest_blogs clearfix" data-href-redirect="{{link}}">
      <dt>
          <a href="{{link}}">
            <span class="latest-blogs-fly-title mv-title">{{title}}</span><br>
            <img class="sponsor-logo" src="{{sponsor.logo.href}}" height="15" width="auto" style="width: auto !important; height: 35px !important;">
            {{summary}}
          </a>
      </dt>
  </div>
...
helpers = this.merge(helpers, Handlebars.helpers); data = data || {};
  ...
  buffer += "<div class=\"views-row views-row-1 views-row-odd views-row-first omniture-tagged omniture-tagged-6 mv-ad\">\n  <div class=\"latest_blogs clearfix\" data-href-redirect=\"";
  ...
  buffer += escapeExpression(stack1)
    + "\">\n            <span class=\"latest-blogs-fly-title mv-title\">";
  ...
  buffer += escapeExpression(stack1)
    ...
    + "\" height=\"15\" width=\"auto\" style=\"width: auto !important; height: 35px !important;\">\n            ";
