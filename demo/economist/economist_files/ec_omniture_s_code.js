...
s.linkInternalFilters="javascript:,economist.com,economistsubscriptions.com,brightcove.com,doubleclick.net";
...
//This will clear the variables that were set in linkTrackVars
//It will also empty s.events

window.clearLinkVars = function () {
  var linkVars = s.linkTrackVars.split(',');
...
//Clear page-level Omniture vars

window.clearOmniVars = function () {
  s.events = '';
  for (var i = 0; i < 75; i++) {
    s['prop' + i] = '';
    s['eVar' + i] = '';
...
function s_doPlugins(s) {
  ...
  s.prop22 = s.getQueryParam('fsrc') + ((typeof s.prop22 === "undefined" || s.prop22 === "") ? "" : ("|" + s.prop22));
  ...
  // Lets get prop13 correct first. If the user was an IP user, due to our cache systems, they may
  // seem anonymous. We account for this by checking if they are anonymous but are in a
  // IP community.
  // @see ec_omniture:_ec_omniture_default_variables
  // @see ec_omniture:ec_omniture_user
  // Handle the subscription not being set.
  if ($.cookie('ec_omniture_user_sub') === null) {
    s.prop13 = 'anonymous';
  }
  else {
    s.prop13 = $.cookie('ec_omniture_user_sub');
  }
  // Set the cookie to bulk-IP if Varnish has set the community.
  if ($.cookie('ec_community') == "10000000000") {
    s.prop13 = 'bulk-IP';
  }
  ...
  // Record other general ad impressions.
  if (typeof window.adImpressions !== "undefined") {
    var cleanImp = [];
    var imp = "";
    for(var aic = 0; aic < window.adImpressions.length; aic++) {
      imp = window.adImpressions[aic];
      if (imp === "") {
        continue;
      }
      cleanImp.push(imp);
    }
    if (cleanImp.length > 0) {
      if (typeof s.prop15 === "undefined" || s.prop15 === "") {
        s.prop15 = cleanImp.join(",");
      }
      else {
        s.prop15 += "," +  cleanImp.join(",");
      }
  ...
  // Due to the way the jsalter_closure fires before nodewords can run,
  // we set the property to track news_keywords here by extracting it
  // from the HTML.
  // Jsclosure writes all the JS out in plain text form before we can ever
  // get to it. We could re-call it or re-call nodewords_page_proc, but this
  // would be double calls for every page load. Too much overhead.
  var metaTags = document.getElementsByTagName("meta");
  var metaSize = metaTags.length;
  var metaCount = 0;
  for (metaCount = 0; metaCount < metaSize; metaCount++) {
    if (metaTags[metaCount].getAttribute("name") === "news_keywords") {
      s.prop24 = metaTags[metaCount].getAttribute("content");
      break;
    }
  }
   ...
   */
  s.Media.trackVars="events,eVar62,eVar12,contextData.bc_tags,contextData.bc_refid,contextData.bc_player,contextData.bc_playertype,contextData.bc_playlist";
  s.Media.trackEvents="event10,event11,event13,event5,event7,event6,event8";
  s.Media.trackMilestones="25,50,75";
  s.Media.segmentByMilestones = true;
  s.Media.trackUsingContextData = true;
  s.Media.contextDataMapping = {
    "a.media.name":"prop27,eVar27",
    "a.media.segment":"eVar62",
    // "a.contentType":"eVar12",
    "a.media.timePlayed":"event5",
    "a.media.view":"event7",
