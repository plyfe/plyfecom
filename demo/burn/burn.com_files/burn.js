...
$j.fn.BURN_carousel = function() {
	...
	load = function(){
		...
		if(data[prev].id == data[next].id){
			if(!$j('.burn_carousel .image.right').length){
				prevcont = false;
			}
			else{
				nextcont = false;
	...
	show_pager = function(){
		if($j('.burn_carousel .image.left').length){
...
$j.fn.closePopup = function() {
    ...
    //hide back cover
    $j('#burn-modal-back').css('z-index','10').hide();
    //reset video
    topmedia.html('');
    //store the content
    ...
    $j('#burn-popup-tandc').hide(); 
    $j(".plusotherbutton ~ div").hide();
    $j('.readmorebtn').html("&#187;&nbsp;"+BURN.translations['more']).next().hide();
    $j('.popup.active').removeClass('active').hide(500,function(){$j(this).popupSize();});   
};

BURN.add_article_actions = function(){
	...
	//grid box hover
    $j('article.burn-article').each(function(){
    	var $this = $j(this),
    ...
    //modal self close
    $j("#burn-modal-back").on('click', function() {$j(this).closePopup();});
...
BURN.load_popup = function(id){
	$j('[id^=burn-popup]').closePopup();
	
	var divid = 'burn-popup_post-'+id;
    ...
    if($j('#'+divid).length == 0){
    	$j.getJSON(
        	...
        	{
        		action: 'popup',
				security: BURN.nonces['popup'],
        		'id' : id
        	},
        	function(response){
        		if(response === -1 || response.success === false) return false; //security check fail
        		if(response.post_type === 'burn_action'){
        			if(response.action_open == 'iframe' && response.action_link !== false){
        				return $j('body').BURN_iframe({action_link: response.action_link});
        			}
        		} else {
        			if(response.login_required === true){
        				BURN.loginManager.open('login');
        			} else {
        				$j("#burn-modal-back").show();        			
        				...
        				$j('#' + divid + '.popup.active .readmorebtn').on('click', function() {
        					...
        					if ($this.next().css("display") == "none") {
        						return $this.html("&#171;&nbsp;"+BURN.translations['less']).next().show(300,function(){$j(this).popupSize();});
        					}
        					return $this.html("&#187;&nbsp;"+BURN.translations['more']).next().hide(300,function(){$j(this).popupSize();});
        				});
        				...
        				if(BURN.open_tandc) BURN.opentandc(id);
...
BURN.Filter = function(){
	this.params = {};
	this.filter; //all the possible filter parameters
	this.filter_filtered;  //the matching filter parameters
	this.posts;
	this.tag_id;
	this.redirect_to_index = BURN.page_template !== "" && BURN.page_template !== "grid.php";
	this.post_type_filters = {
        'burn_date' : ['what', 'when', 'where'],
        'burn_unit' : ['what', 'where']
    };
		...
		$j.getJSON(
			...
			function(response){
				...
				if(response.filter_html !== ''){
					$j('.burn_filter').html(response.filter_html);
				}
				if (me.posts.length === 0) {
					minHeight = "0";
				} 
				$j('.burn_articles').html(response.content).css("min-height", minHeight);
				me.apply_filter();
				BURN.set_permalink();
				BURN.add_article_actions();
			}
		);
	};
		...
		if(this.params){
			$j.each(me.post_type_filters, function(post_type, filters){
		    	for(var k = 0;  k < filters.length; k++){
		    		...
		    		$j('.'+ post_type +'_filter_' + filters[k] + ' .title').html(BURN.translations[filters[k]]);
...
BURN.pos_elements = function(postype){
	var sidebar = $j("#sidebar"),
		popup = $j(".active.popup"),
		iframe = $j("#burn-iframe"),
...
BURN.promo = function(action,postid){
	if('assign'==action && !$j('#accepttandc').is(':checked')){
		$j('.burn-promo-error').slideDown(200,function(){
			$j(this).children('.burn-promo-message-close').click(function(){
				$j('.burn-promo-error').fadeOut(500,function(){
				});
			});
		});
		return false;
	...
	$j.getJSON(
		...
		function(response){
			...
			$j('.burn-promo-success').show(1,function(){
				$j(this).children('.burn-promo-message-close').click(function(){
					$j('.burn-promo-success').fadeOut(500,function(){
...
$j(document).ready(function(){
	...
	BURN.post_filter = new BURN.Filter();
	BURN.post_filter.init();
	BURN.add_article_actions();
	BURN.set_permalink();
	
	$j("input.burn_search").on("focus",function() {
		if($j("input.burn_search").attr("edited") !== "true"){
			$j("input.burn_search").attr("edited","true");
			$j("input.burn_search").val("");
		}
	});
	$j("input.burn_search").on("blur",function() {
		if($j("input.burn_search").val() === ""){
			$j("input.burn_search").val($j("input.burn_search").attr("searchtext"));
			$j("input.burn_search").attr("edited","false");
		}
	});
	$j(".searchbox form").submit(function(){
		if($j("input.burn_search").attr("edited") !== "true" || $j("input.burn_search").val() === $j("input.burn_search").attr("searchtext")){
			return false;
		}
		var val = $j("input.burn_search").val(); 
		var pattern = XRegExp('[\\p{n}|\\p{L}]{4,}');
		if(!pattern.test(val)){
			$j("input.burn_search").val($j("input.burn_search").attr("searchtext"));
			$j("input.burn_search").attr("edited","false").blur();
			return false;
		}
	...
	// scrolling
	$j(window).scroll(function(){BURN.pos_elements('scroll');});
	$j(window).resize(function(){BURN.pos_elements('resize');});
	...
	$j('#popup-tandc-close').click(function(){
		...
		$j("#burn-modal-back").css('z-index','10');
	});
});