(function($) {
  ...
  function postParentMessage(msg) {
    postRawParentMessage(window.grvSiteGuid + '|' + window.grvPlacement + '|' + window.grvUserGuid + '|' + msg);
  }

  function postRawParentMessage(msg) {
    if(postMessageSupported && window.grvFrameUrl && parent !== window)
      parent.postMessage(msg, window.grvFrameUrl);
  }
  ...
  function grvUpdateHeight() {
    ...
    postParentMessage('setHeight:' + (window.grvUseDynamicHeight ? $widget.outerHeight(true) : window.grvStaticHeight));
  }
  ...
  (function() {
    ...
    var onImageError = function(width, height) {
      ...
      if(width && height) {
        var $img = $(this),
            brokenImgThumb = (location.protocol == 'https:' ? 'https://grvaol-a.akamaihd.net' : 'http://a.rtb.grvcdn.com') + '/t/' + width + 'x' + height + '/Center/?url=' + encodeURIComponent(window.grvBrokenImgUrl);
  ...
  function checkFullWidthRatio($img) {
    ...
    if(heightRatio > 1.5) {
      if(!$parent.hasClass('img_magic')) {
        ...
        for(var i=2; i<=8; ++i)
          $baseClone.clone().css('left', '-' + i + '%').addClass('fade_' + i);
  ...
  function grvHorizontalSpace() {
    ...
    widget_width = $widget.innerWidth()-($('.grv_article').length-1)*5;
  ...
  function bindArticleHandlers($grvparent) {
    ...
    if(/[&?]cc(&|$)/.test(location.search || '') && window.grvIsIframeWidget && postMessageSupported) {
      // Article link click
    ...
    // Partner not capturing clicks; normal click behavior
      ...
      // Non-ProBoards
        ...
        if(window.grvDoAolOmniture) {
          ...
          $forwardingLinks.click(rewriteHref).bind('contextmenu', rewriteHref);
      }
    }
  ...
  function initDotdotdot() {
    ...
    if(truncateArticleContent) {
      ...
      $filtered.length && $articleContents.filter(elemHasDim).dotdotdot();
    }
  }

  function updateDotdotdot() {
    // Only update dotdotdot if elements have dimensions
    var $filtered = $articleTitles.add($articleContents).filter(elemHasDim);
    if($filtered.length) {
  ...
  function grvInitInnerScroll() {
    if(window.grvUseInnerScroll && $grvScrollableHandle) {
      ...
      // NanoScroller ready
        ...
        if(!window.grvIsIframeWidget && !$widget.is(':visible')) {
