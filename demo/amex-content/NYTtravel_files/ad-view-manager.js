    ...
    var AdViewManager = BaseView.registerView('ad-view-manager').extend({
        ...
        **/
        render: function(args) {
            ...
            // return if there is no associated ad model and ad element
            if (!ad || typeof ad.get === 'undefined' || !args.el) {
                return;
            }
            ...
            if (isInlineSafe) {
                args.el.html(args.el.html() + adCreative);

            // place ads that are not labeled "safe" in an iframe
            } else if (args.el.find('iframe').length === 0) {
