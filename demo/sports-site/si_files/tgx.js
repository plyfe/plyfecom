...
function TiiAdConfig(sitename) {
	...
	this.setTacodaTracking = TiiAdConfigSetTacodaTracking;
        ...
        this.useQuantcastTracking = true;
...
function TiiAdConfigSetQuantcastTracking(value) {
	this.useQuantcastTracking = value;
}
...
function TiiAdFactorySlotRenderEnded(callback, adslots) {
        try {
            googletag.pubads().addEventListener('slotRenderEnded', function(event) {
                ...
                if (event.isEmpty) {
                    document.getElementById(divId).style.disaply = "none";
...
var TIICONSTANTS = {
    ...
    'bk_enabled_root_domains': 'content.timeinc.net,people.com,peoplestylewatch.com,celebritybabies.com,peoplepets.com,realsimple.com,ew.com,peopleenespanol.com,essence.com,instyle.com,time.com,timeforkids.com,sportsillustrated.cnn.com,si.com,sikids.com,golf.com,fannation.com,allyou.com,coastalliving.com,cookinglight.com,myrecipes.com,southernliving.com,sunset.com,thisoldhouse.com,myhomeideas.com,health.com',
    'bk_disabled_root_domains': 'eskyguide.com,foodandwine.com,travelandleisure.com,departures.com,executivetravelmagazine.com',
    'bk_id': '13731',
    'bk_pixel_limit': '6',
...
function TgxAdBridgeClearRefreshSlots(slots, slotkeys) {
    var slotsToRefresh = new Array();
    for (i = 0; i < slots.length; i++) {
