...
var _jsmd_default={
	...
	dictionary: {
		init: {
			...
			"page.url_section[0]":				"gJObj|cnn_metadata,url_section[0]",	//prop41,eVar44
			"page.url_section[1]":				"gJObj|cnn_metadata,url_section[1]",	//prop42,eVar45
			...
			"business.si.column":					"gJObj|cnn_metadata,column",		//prop,eVar13,8,9
			...
			"business.si.tcm_friendly_name":		"gJObj|cnn_metadata,page_headline"				//prop54,eVar54
			,
			...
			postinit: function() {
				...
				/* set values for prop41,eVar44 if not set in cnn_metadata */
				if (this.get("page.url_section[0]") == null) {
					this.set("page.url_section[0]",_jsmd.plugin.gADBPURL("path",1)); // first directory
				}
				if (this.get("page.url_section[0]") == "") {
					this.set("page.url_section[0]","home");
				}

				//* set  values for prop42,eVar45 if not in the cnn_metadata */
				if (this.get("page.url_section[1]") == null) {
					this.set("page.url_section[1]",_jsmd.plugin.gADBPURL("path",2)); // second directory
				}

				/* set value for prop28/eVar28 if not in the cnn_metadata */
				...
				if (((siHost.indexOf("sportsillustrated") > -1) || (siHost.indexOf("preview") > -1)) && !(window.location.pathname.indexOf("/vault/") > -1)) {
					if (ch != null && ch != "") {
			...
			vendors: [
				...
				},
				{
					...
					dynamic_actions: {
						...
						"registration-completed":		{ ignore: true }
					}
			...
			*/
			...
			si: {
				account: function() {
					...
					if ((host.indexOf("preview") > -1) || (host.indexOf("vaultp1refcomp1") > -1) || (host.indexOf("webp1refcache1") > -1)) {
					...
					//accountName = "sidsidev"; // FOR TESTING ONLY :: TO DO - Develop a better solution
					return accountName;
				},
				settings: {
					...
					"linkInternalFilters":				"javascript:,sportsillustrated,.si.com,turner.com,twackle.com,apps.facebook.com,sifantasy.secondthought.com,st-sifantasy.staging.catalyticgroup.com,sportstechinc.com,amazonaws.com,extramustard.si.com,vbplayer,traffic.outbrain.com",
				...
				variablemap: {
					...
					"business.si.social_type":			["prop24","eVar24"],
					"business.si.gallery_info":			["prop25","eVar25"],
					"business.si.tcm_friendly_name":		["prop54","eVar54"],
				...
				postmap: function() {
					...
					if (document.title.match("404")||_jsmd.plugin.gJObj("cnn_metadata","type") == 'errorPage'){
					...
					if (this.config.map.isDynamic != null && this.config.map.isDynamic.indexOf("preroll") > -1) {
						// multi-suite video
						this.v.contextData['bc_title'] = this.v.prop29;
						this.v.contextData['bc_franchise'] = this.v.prop5;
						this.v.contextData['bc_event'] = this.v.events;
						this.v.contextData['bc_site'] = 'si';
						this.v.contextData['bc_channel'] = this.v.eVar27;
					...
					} else if (this.config.map.isDynamic != null && this.config.map.isDynamic.indexOf("start") > -1) {
						// multi-suite video
						this.v.contextData['bc_title'] = this.v.prop29;
						this.v.contextData['bc_franchise'] = this.v.prop5;
						this.v.contextData['bc_event'] = this.v.events;
					...
					/* livefyre click tracking - add pagename, fix for event26 */
					if (this.config.map.isDynamic != null && this.config.map.isDynamic.indexOf("livefyre-click") > -1) {
						this.v.eVar26 = this.get("page.name");
					};
					/* registration submit click trackin g- add pagename, fix for event26 */
					if (this.config.map.isDynamic != null && this.config.map.isDynamic.indexOf("registration-completed") > -1) {
						this.v.eVar26 = this.get("page.name");
					};
				}
			},
			adbp: {
				filters: {
					...
					"livefyre-click":	{ include: ["page.section[0]","code.version"] },
					"registration-completed":	{ include: ["page.section[0]","code.version"] }
				},
				settings: {
					"trackInlineStats":				true,
					"linkLeaveQueryString":			false
			...
			"adbp-video": {
				...
				eventmap: {
					...
					"video.subsequent_start":		["event50"],
					"video.start":					["event32"],
					"video.complete":				["event33"],
					"video.autostart":				["event34"],
					"video.preroll":				["event35"],
	...
	plugins: {
		...
		/* promo.external.id : campaign */
		gSICampaign:function(){
			var xCampaign = _jsmd.plugin.gQuery("xid","sr"); //get the query string
			/* call Omniture 'getValOnce' plugin */
			var s = this.vendor.Adobe.plugins();
			return s.getValOnce(xCampaign,"s_campaign",0);
		},
		...
		gSIVidPlayerLoc:function() {
			var path = _w.location.pathname.substring(1);
			...
			var swimRes;
			var vidPlayerLoc;
			if(/extramustard/.test(hostName) == true){
				vidPlayerLoc = "extramustard";
			} else if(/swimdaily/.test(hostName) == true){
				vidPlayerLoc = "swimdaily";
			} else {
				...
				// Blog right rail video location
				var rightRailParent = "";
				if(window.location != window.parent.location){
					rightRailParent = document.referrer;
					if (rightRailParent.match(/tracking\.si\.com\/\?sct\=nbar_blogs_dd/)) {
						vidPlayerLoc = 'siwire'
					} else if(rightRailParent.match(/extramustard\.si\.com/)) {
						vidPlayerLoc = 'extramustard';
					} else if (rightRailParent.match(/swimdaily\.si\.com/) || parentUrl.match(/swimdaily\.si-wpdev\.us/)) {
						vidPlayerLoc = 'swimdaily';
					} else if (rightRailParent.match(/nfl\.si\.com/) || parentUrl.match(/nfl\.si-wpdev\.us/)) {
						vidPlayerLoc = 'nfl';
					} else if (rightRailParent.match(/mlb\.si\.com/) || parentUrl.match(/mlb\.si-wpdev\.us/)) {
						vidPlayerLoc = 'mlb';
					} else if (rightRailParent.match(/nba\.si\.com/) || parentUrl.match(/nba\.si-wpdev-us/)) {
						vidPlayerLoc = 'nba';
					} else if (rightRailParent.match(/college-football\.si\.com/) || parentUrl.match(/college-football\.si-wpdev\.us/)) {
						vidPlayerLoc = 'college-football';
					} else if (rightRailParent.match(/nhl\.si\.com/)) {
						vidPlayerLoc = 'nhl';
					} else if (rightRailParent.match(/college-basketball\.si\.com/) || parentUrl.match(/college-basketball\.si-wpdev\.us/)) {
						vidPlayerLoc = 'college-basketball';
					} else if (rightRailParent.match(/mmqb\.si\.com/) || parentUrl.match(/mmqb\.si-wpdev\.us/)) {
						vidPlayerLoc = 'mmqb';
					}
				}
			};
 ...
 */
	 ...
	 */
		...
		gMultiSuiteVideo:function(){ // multi-suite video
			var s_account_video='timeglobalvideo';
			if(/(file:\/\/\/|localhost|sipreview\.turner\.com|preview\.sportsillustrated\.turner\.com|preview\.dev|vaultp1refcomp1|webp1refcache1)/.test(location.href)){s_account_video='timeglobalvideodev';}
			return s_account_video;
		},
		vendor: {
			Adobe: {
				plugins: function(s) {
	...
	actions: {
		...
		"video-start": function(data, map) {
			...
			} else if(v.isSubsequent == true) {
				this.push("page.events","video.subsequent_start");	//event50
				this.set("link",{name: "video-subsequent_start: "+ v.headline, type: "o"});
			} else {
		...
		// livefyre-click used for all user action
		"livefyre-click": function(data) {
			if (typeof(data.action_type) != 'undefined' && data.action_type.search(/comment/)!=-1) {
				this.push("page.events","social.comment_submit");	//event25
				this.set("link",{name: "social: "+data.action, type: "o"});
			} else {
				this.push("page.events","social.interaction");	//event24
				this.set("link",{name: "user-action: "+data.action, type: "o"});
			}
		...
		"registration-completed": function(data) {
			this.push("page.events","registration.complete");		//event28;
			this.set("action","link");
			this.set("link",{name: "registration-complete", type: "o"});
			this.send();
		}
	}
	...
	prvDefaultVendorSpecificTemplate=_jsmd_default.vendor||{
		...
		"Adobe SiteCatalyst H-code": {
			...
			initialize: function(accountf,vendorAction) {
				...
				if(typeof(accountf)=="function") {a=accountf.call(this)||"IGNORE";}
				o=s_gi(a);
				o.usePlugins = false;o.multiSuite = false;
				delete o.pageName;
				if(va==="link") {
					var lnk=this.get("link");
					var t=lnk.type,n=lnk.name;
					if((t==="o")&&(/^video/i.test(n))){ // multi-suite video
					o.usePlugins = true;o.multiSuite = true;
					}
				...
				o.doPlugins=function() {
					if(o.multiSuite) {o.sa(a+","+_jsmd.plugin.gMultiSuiteVideo());o.multiSuite = false;}
					else {o.sa(a);}
				};
				o.products=null;
				return o;
...
/* Added 03/28/2013  - re-fires comScore beacon with each dynamic page view           *
 * The cs (TiiCsBeacon) object is instantiated in the global ads library (tii_ads.js) */
function sendComScoreSyntheticRefresh(){
	try{
		if(cs.enabled) {
			cs.csFireBeacon();
		}
...
var NielsenHybridTag=function(){
	function doNielsen(a) {
	    ...
	    d.src = ["", escape(a), "&cg=0&cc=1&si=", escape(window.location.href), "&rp=", escape(document.referrer), "&ts=compact&rnd=", (new Date()).getTime()].join('');
  }
