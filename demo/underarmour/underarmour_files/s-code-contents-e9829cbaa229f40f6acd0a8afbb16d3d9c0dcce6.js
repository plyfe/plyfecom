...
/* Plugins */
AppMeasurement.prototype.setEvent = function(event, isLink){
  var s = this, evts;
  s.events = s.events || '';
  evts = s.events+',';
  // only set the event if it is not already set
  if(evts.indexOf(event+',')==-1 && evts.indexOf(event+':')==-1 && evts.indexOf(event+'=')==-1){
    s.events = s.events + (s.events.length > 0 ? ',' : '') + event;
  }
  if(isLink === true){
    s.linkTrackEvents = (s.linkTrackEvents || '') + ',' + event;
  }
};
AppMeasurement.prototype.removeEvent = function(event){
  var s = this, evts;
 ...
 */
AppMeasurement.prototype.crossVisitParticipation=new Function("v","cn","ex","ct","dl","ev","dv",""
 ...
 */
AppMeasurement.prototype.split=new Function("l","d",""
+"var i,x=0,a=new Array;while(l){i=l.indexOf(d);i=i>-1?i:l.length;a[x"
+"++]=l.substring(0,i);l=l.substring(i+d.length);}return a");
/*
 * Utility Function: s.join: 1.0 - s.join(v,p)
 */
AppMeasurement.prototype.join = new Function("v","p",""
 ...
 */
AppMeasurement.prototype.repl=new Function("x","o","n",""
+"var i=x.indexOf(o),l=n.length;while(x&&i>=0){x=x.substring(0,i)+n+x."
+"substring(i+o.length);i=x.indexOf(o,i+l)}return x");
/*
* Plugin: getValOnce_v1.1
*/
AppMeasurement.prototype.getValOnce=new Function("v","c","e","t",""
...
*/
AppMeasurement.prototype.apl=new Function("l","v","d","u",""
 ...
 */
AppMeasurement.prototype.getTimeParting=new Function("t","z","y","l",""
 ...
 */
AppMeasurement.prototype.channelManager=new Function("a","b","c","d","e","f",""
...
/* non-custom list */
AppMeasurement.prototype.seList="altavista.co|q,r|AltaVista>aol.co.uk,search.aol.co.uk|query"
...
AppMeasurement.prototype._channelPattern="Product Listing Ads|PLA>Re-Targeting Display Ads|RT>Catalog|catalog>Email|EM>Email|em>Affiliates|AF>Banner|BNR:>Social Media|SM>Shopping Engines|SE>Display Advertising|DA>Paid Search|PS>Paid Search|ps>Google Product Search|GPS>Referrals|REF";

var s_account = 'underarmourcom';
...
s.doPlugins = function(s){
  ...
  /* Channel Manager */
  s.channelManager('cid,fb_ref', ':', '', '', 's_dl');
  if(s._campaign){
    ...
    if((s._channel=='Referrers')||(s._channel=='Other Natural Referrers')){
      ...
      s._campaignID='REF|'+s._referringDomain[0];

      switch(s._campaignID){
        case ("REF|www.facebook.com"):
          s._channel="Social Media";
          s._campaignID="SM|www.facebook.com";
          break;
        case ("REF|static.ak.facebook.com"):
          s._channel="Social Media";
          s._campaignID="SM|static.ak.facebook.com";
          break;
        case ("REF|pinterest.com"):
          s._channel="Social Media";
          s._campaignID="SM|www.pinterest.com";
          break;
        case ("REF|twitter.com"):
          s._channel="Social Media";
          s._campaignID="SM|twitter.com";
          break;
        case ("REF|www.youtube.com"):
          s._channel="Social Media";
          s._campaignID="SM|www.youtube.com";
          break;
      }
    }
    ...
    if(s.campaign && typeof s.campaign != 'undefined'){
      ...
      if (s._channel!='')
        s.eVar59=s._channel;
      s.eVar18='External Campaigns';
    }
  }
  if(s.campaign){
	  s.eVar41 = s.crossVisitParticipation(s.campaign,'v41_cvp','60','5','>','purchase');
  }
  
  /* before send - process data that needs to go after everything else */
  if(s.beforeSend.length > 0){
    for(var j=0; j<s.beforeSend.length; j++){
      if(typeof s.beforeSend[j] == 'function'){
        s.beforeSend[j]();
      }
    }
    s.beforeSend = [];
  ...
  /* callback - fires after data is sent */
  s.q = function(){
    _satellite.notify('Adobe Analytics callback executed', 1);
    /* see if we have anything to clear in the s.cb array */
    for(var i=0; i<s.cb.length; i++){
      if(typeof s.cb[i] == 'function'){
        s.cb[i]();
      }
    }
    s.cb = [];
    ...
    /* set previous page */
    if(s.pageName){
      _satellite.setCookie('prev_page', s.pageName);
    }
  };
};